...
const { storeEmbeddings, clearStore } = require('../services/vector/vector.service');
 ...
 * Useful for callers (e.g. the Telegram bot) that want each PDF to be
 * generated from in isolation, without context bleeding in from earlier uploads.
 */
exports.clearStore = async (req, res) => {
    try {
        await clearStore();
        return res.status(200).json({ success: true, message: 'Vector store cleared.' });
    } catch (error) {
        console.error('Error clearing vector store:', error);
        return res.status(500).json({ success: false, message: 'Failed to clear vector store.', error: error.message });
    }
};
